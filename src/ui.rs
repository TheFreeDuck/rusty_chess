use crate::{chess::{self, piece}, draw::WindowParameters};
...
impl ChessBoard {
    ...
    pub fn render(&mut self, window_parameters: &WindowParameters) {
        ...
        match self.held_piece {
            Some(held_piece) => {
                match self.squares.get(&held_piece){
                    Some(square) => {
                        if let Some(ref piece) = square.graphics_piece{
                            piece.render(window_parameters, square);
                        }
    ...
    pub fn update(&mut self, window_parameters: &WindowParameters) {
        ...
        for ((_i, _j), square) in &self.squares {
            if let Some(ref piece) = square.graphics_piece{
                match piece{
                    GraphicsPiece::Pawn { texture: _, color: _, x, y } |
                    GraphicsPiece::Knight { texture: _, color: _, x, y } |
                    GraphicsPiece::Bishop { texture: _, color: _, x, y } |
                    GraphicsPiece::Rook { texture: _, color: _, x, y } |
                    GraphicsPiece::Queen { texture: _, color: _, x, y } |
                    GraphicsPiece::King { texture: _, color: _, x, y } => {
                        let is_hovered = mouse_x >= *x && mouse_x <= x + self.width && mouse_y >= *y && mouse_y <= y + self.width;
