...
impl Piece {
    ...
    fn is_legal_pawn_move(from: Coordinate, to: Coordinate, board: &Board, color: Color) -> bool {
        let is_capture = match board.squares[to.x_usize()][to.y_usize()] {
            Some(_) => {
                board.squares[to.x_usize()][to.y_usize()]
                    .unwrap()
                    .get_color()
                    != color
            }
        ...
        let can_double_move =
            from.y == 1 && color == Color::White || from.y == 6 && color == Color::Black;
    ...
    fn is_legal_knight_move(from: Coordinate, to: Coordinate, board: &Board, color: Color) -> bool {
